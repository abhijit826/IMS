...
const ContentView = ({
  ...
  setView, // Add setView here
}) => {
  // Memoize the calculation for warehousesWithQuantities
  const warehousesWithQuantities = useMemo(() => {
  ...
  }, [warehouses, warehouseQuantities]); // Dependency array
    ...
    >
        ...
        <>
          ...
          {!barLoading && !barError && (
            <ItemChart
              ...
              pieData={{}}
              theme={theme}
              setView={setView} // eslint-disable-line no-undef
              setFilter={handleFilterChange} // Ensure setFilter is passed
            />
          )}
        </>
      )}
      {view === 'pie' && canViewCharts && (
        <>
          ...
          {!pieLoading && !pieError && (
            <ItemChart
              type="pie" barData={{}} pieData={pieData} theme={theme} setView={setView} setFilter={handleFilterChange} // eslint-disable-line no-undef
            />
          )}
        </>
      )}
