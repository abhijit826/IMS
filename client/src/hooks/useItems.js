...
export const useItems = (isAuthenticated, view, apiUrl, itemsPerPage) => {
  ...
  const { data: itemsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`${apiUrl}/api/items`, { params });
      return res.data;
    },
    ...
    onError: (err) => { // Add error handling
      ...
      message.error(`Failed to fetch items: ${err.response?.data?.message || err.message}`);
    }
  });
  ...
  return {
    ...
    handleSort,
    handleFilterChange,
    setSearchTerm, // <-- Return setSearchTerm
    handlePageChange,
  };
};